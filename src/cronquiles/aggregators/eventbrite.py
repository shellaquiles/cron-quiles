...
class EventbriteExtractor:
    ...
    def extract_from_url(self, url: str) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Extracted {len(mx_events)} events from {url} (Total found: {len(events)})")
            return mx_events
    ...
    def _is_event_type(self, node_type: str) -> bool:
        ...
        if not node_type:
            return False
        event_types = {'Event', 'BusinessEvent', 'SocialEvent', 'EducationEvent', 'PublicationEvent'}
    ...
    def _parse_single_event(self, data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # Si es online
            if data.get('eventAttendanceMode') == 'https://schema.org/OnlineEventAttendanceMode':
                location_str = "Online"
